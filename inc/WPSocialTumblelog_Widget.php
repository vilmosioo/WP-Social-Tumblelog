...
class WPSocialTumblelog_Widget extends WP_Widget{
	...
	function __construct($args = array()) {
		$args = array_merge ( array(
			"title" => 'Tumblelog',
			"description" => 'Displays the tumblelog ',
			"class" => 'wpsocialtumblelog_widget'
		), $args );
		
		$this->slug = WPSocialTumblelog_Utils::generate_slug($args['title']);
		$this->title = $args['title'];
		$this->description = $args['description'];
		$this->class = $args['class'];
		$this->fields = array('Count');
		parent::__construct( false, $this->title, array('description' => $this->description) );
	}

	function WPSocialTumblelog_Widget() {
		$widget_ops = array( 
	...
	*/
	function widget( $args, $instance ) {
		...
		$count = empty($this->fields[0]) && is_numeric($this->fields[0]) ? $this->fields[0] : 5;
		echo do_shortcode( "[tumblelog count=$count]" );
